import { ChevronDown, ChevronRight, Clock, Copy, Download, Trash2 } from "lucide-react";
			...
			<div className="flex items-center gap-3 p-3">
				...
				<div className="flex-1 min-w-0">
					...
					<div className="flex items-center gap-2 text-xs text-glass-muted">
						<span className="flex items-center gap-1">{metadataParts.join(" • ")}</span>
					</div>
				</div>
				...
				<div className="px-3 pb-3 pt-0 border-t border-white/5 animate-in fade-in slide-in-from-top-2 duration-200">
					<div className="pt-3 space-y-2">
						...
						<div>
							...
							<p className="text-xs text-glass-secondary">{new Date(entry.timestamp).toLocaleString()}</p>
						</div>
					</div>
				</div>
			)}
