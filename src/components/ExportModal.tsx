...
export function ExportModal({ isOpen, onClose, data, onExportComplete }: ExportModalProps) {
  ...
  useEffect(() => {
    if (data.length > 0 && isFilenameAutoGenerated && !filename) {
      setFilename(generateFilename(selectedFormat));
    }
  }, [data, isFilenameAutoGenerated, filename, selectedFormat]);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      await onExportComplete?.(data, selectedFormat, selectedAction, selectedAction === 'file' ? filename : undefined);
      onClose();
    } catch (err) {
    ...
    >
      <div className="glass-heavy w-full max-w-md rounded-2xl shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="flex items-center justify-between p-5 border-b border-white/10">
          <h2 className="text-lg font-semibold text-glass-primary">Export Options</h2>
          <button
            onClick={onClose}
