 ...
 */
export async function saveHistoryEntry(entry: Omit<HistoryEntry, 'id' | 'dataSize'> & { data: ExtractedData[] }): Promise<HistoryEntry | null> {
  try {
...
export async function searchHistory(query: SearchQuery): Promise<HistoryEntry[]> {
  try {
    ...
    if (query.domains && query.domains.length > 0) {
      ...
      entries = entries.filter(entry =>
        entry.domains.some(d => searchDomains.has(d.toLowerCase()))
      );
    }
